...
/* MZ 05.09.2024  - Start */
#define RPLANCK 1.054571817e-34            // Reduced Planck constant in J/s
#define LIGHTSPEED 299792458.0             // Speed of light in m/s
/* MZ 05.09.2024 - End */
#define EPSILON0 8.854187817e-12           // Epsilon0 in SI
#define DEBYE 3.336e-30                    // Debye in SI
...
#define TIME_UNIT 1.0e-15                  // The unit of measure for the time is the femtosecond
...
void Interp2D_3C_data(const string& filename, vector<double> &vecA, vector<double> &vecP,  vector<double> &vecD){
    ifstream fin(filename);
    if (!fin.good()){
        cout << endl << " ### ERROR: File " << filename << " not found ###" <<  endl;
        bad_termination();
    ...
    vector<long long int> suppA, suppP;
    for (long long int i=0; i < vecA.size(); i++){
        if (vecA[i] < vecX[0] || vecA[i] > vecX[n1-1]){
            suppA.push_back(-1);
        }
        else{
            for (long long int ii=0; ii < vecX.size()-1; ii++){
                if (vecA[i] > vecX[ii] && vecA[i] < vecX[ii+1]){
                    suppA.push_back(ii);
                }
            }
        }
    }
    for (long long int i=0; i < vecP.size(); i++){
        if (vecP[i] < vecY[0] || vecP[i] > vecY[n1-1]){
            suppP.push_back(-1);
        }
        else{
            for (long long int ii=0; ii < vecY.size()-1; ii++){
                if (vecP[i] > vecY[ii] && vecP[i] < vecY[ii+1]){
                    suppP.push_back(ii);
                }
            }
        }
    ...
    double val;
    long long int nX, nP, nT;
    for (long long int i=0; i < vecA.size(); i++){
        if (suppA[i] == -1){
            for (long long int ii=0; ii < vecP.size(); ii++){
                vecD.push_back(0);
            }
        }
        else{
            ...
            for (long long int ii=0; ii < vecP.size(); ii++){
                if (suppP[ii] == -1){
                    ...
                    nP = suppP[ii];
                    nT = nX * vecP.size() + nP;
                    val += vecZ[nT]*(vecX[nX+1]-vecA[i])*(vecY[nP+1]-vecP[ii]);
                    val += vecZ[nT+vecP.size()]*(vecA[i]-vecX[nX])*(vecY[nP+1]-vecP[ii]);
                    val += vecZ[nT+1]*(vecX[nX+1]-vecA[i])*(vecP[ii]-vecY[nP]);
                    val += vecZ[nT+vecP.size()+1]*(vecA[i]-vecX[nX])*(vecP[ii]-vecY[nP]);
                    val /= (vecX[nX+1]-vecX[nX])*(vecP[nP+1]-vecP[nP]);
                    vecD.push_back(val);
                }
            }
        }
...
QSLE::QSLE(const string& input_name)
{
    ...
    /*MZ 05.09.2024*/ mu01_file = "";
    /*RC 01.10.2024*/ wrad_file = wnorad_file = "";
    ...
    for (string line; getline(fin, line);){
        ...
        if (iss >> x >> y ){
            ...
            /*MZ 05.09.2024*/else if (x == "mu01"){
            /*MZ 05.09.2024*/    mu01_file = y;
            /*MZ 05.09.2024*/}
            /*RC 01.10.2024*/else if (x == "wrad"){
            /*RC 01.10.2024*/    wrad_file = y;
            /*RC 01.10.2024*/}
            /*RC 01.10.2024*/else if (x == "wnorad"){
            /*RC 01.10.2024*/    wnorad_file = y;
            /*RC 01.10.2024*/}
            else if (x == "prefix"){
                base_name = y;
            }
    ...
    /* MZ 05.09.2024  - Start */
    if (!mu01_file.empty()){
        cout << "- Input mu01 file                    : " << mu01_file << endl;
    }
    /* MZ 05.09.2024  - End */
    /* RC 01.10.2024  - Start */
    if (!wrad_file.empty()){
        cout << "- Input radiative rates file         : " << mu01_file << endl;
    }
    if (!wnorad_file.empty()){
        cout << "- Input nonradiative rates file      : " << mu01_file << endl;
    }
    /* RC 01.10.2024  - End */
    cout << "- Input mu01 file                    : " << mu01_file << endl;
...
void QSLE::set_potential_vectors()
{
    ...
    /*MZ 05.09.2024*/E1mE0third  = vector<double>(angle_points);
    for (long long int i=0; i< angle_points; i++){
        deltaE_01[i] = GS_fun(angle_grid[i]) - ES_fun(angle_grid[i]);
        FG[i]= - GS_fun.deriv(1, angle_grid[i]);
        FE[i]= - ES_fun.deriv(1, angle_grid[i]);
        /*MZ 05.09.2024*/E1mE0third[i] = -deltaE_01[i] / energy_conversion * EV;
        /*MZ 05.09.2024*/E1mE0third[i] *= E1mE0third[i] * E1mE0third[i];
    }
}
...
/* MZ 05.09.2024 - Start */
/*************************/
/* Setting dipole moment */
/*************************/
void QSLE::set_mu01()
{
    if (!mu01_file.empty()){
        cout << "=======================================================================" << endl;
        cout << "Reading the 0<--1 transition dipole moment from file " << mu01_file << endl;
        cout << "=======================================================================" <<  endl << endl;
        get_2C_data(mu01_file, supp_MU01, MU01);
        tk::spline MU01_fun(supp_MU01, MU01);
        sqMU01 = vector<double>(angle_points);
        for (long long int i=0; i< angle_points; i++){
            sqMU01[i] = MU01_fun(angle_grid[i]) * MU01_fun(angle_grid[i]) * DEBYE * DEBYE;
        }
    }
}
/* MZ 05.09.2024 - End */
...
/* RC 01.10.2024 - Start */
/*************************************/
/* Setting extra radiative constants */
/*************************************/
void QSLE::set_wrad()
{
    if (!wrad_file.empty()){
        cout << "=======================================================================" << endl;
        cout << "Reading extra radiative rates from file " << wrad_file << endl;
        cout << "=======================================================================" <<  endl << endl;
        get_2C_data(wrad_file, supp_wrad, wrad_prov);
        tk::spline wrad_fun(supp_wrad, wrad_prov);
        wrad = vector<double>(angle_points);
        for (long long int i=0; i< angle_points; i++){
            wrad[i] = wrad_fun(angle_grid[i]);
        }
    }
}
/**********************************/
/* Setting nonradiative constants */
/**********************************/
void QSLE::set_wnorad()
{
    if (!wnorad_file.empty()){
        cout << "=======================================================================" << endl;
        cout << "Reading extra nonradiative rates from file " << wrad_file << endl;
        cout << "=======================================================================" <<  endl << endl;
        Interp2D_3C_data(wnorad_file, angle_grid, mom_grid, wnorad);
    }
}
/* RC 01.10.2024 - End */
...
void QSLE::compute_LRBF_matrix()
{
    ...
    for (long long int i=0; i < number_of_points; i++){
        ...
        if (!wnorad_file.empty()){
            if (wnorad[i] != 0) {
                ...
                supp = i - i % mom_points + iteration;
                GammaM(i, supp + number_of_points) += wnorad[i];
                GammaM(i + number_of_points, i + number_of_points) -= wnorad[i];
            }
        }
        /* RC 01.10.2024 - End */
    }
    ...
    // Radiative decay
    /*MZ 05.09.2024*/add_radiative_decay();
...
/* MZ 05.09.2024 - Start */
/**********************************************/
/* Vertical radiative decay as implemented in */
/* eq. 59 of DOI:10.1021/acs.jpca.1c06165     */
/**********************************************/
void QSLE::add_radiative_decay()
{
    long long int q_idx, p_idx;
    double mult = 4.0 / 3.0 ;
    mult /= RPLANCK * RPLANCK * RPLANCK * RPLANCK;
    mult /= LIGHTSPEED * LIGHTSPEED * LIGHTSPEED;
    mult /= 4.0* M_PI * EPSILON0 * TIME_UNIT;
    // This loop should be optimized for cache trashing
    if (wrad_file.empty()){
        for (long long int i=0; i < number_of_points; i++){
            q_idx = i / mom_points;
            p_idx = i % mom_points;
            GammaM(i, i + number_of_points) += (mult * E1mE0third[q_idx] * sqMU01[q_idx]);  
            GammaM(i + number_of_points, i + number_of_points) -= (mult * E1mE0third[q_idx] * sqMU01[q_idx]);
        }
    }
    else{
        for (long long int i=0; i < number_of_points; i++){
            q_idx = i / mom_points;
            p_idx = i % mom_points;
            GammaM(i, i + number_of_points) += (mult * E1mE0third[q_idx] * sqMU01[q_idx] + wrad[q_idx]);  
            GammaM(i + number_of_points, i + number_of_points) -= (mult * E1mE0third[q_idx] * sqMU01[q_idx] + wrad[q_idx]);
        }
    }
    return;
}
...
void QSLE::time_evolution()
{
    ...
    integral_X=zeros(angle_points*2);
    X = angle_grid;
    Y = mom_grid;
    ...
    #pragma omp parallel
    {
        ...
        long long int start_point, end_point, slice_points;
        long long int start_angle, end_angle;
        start_angle = ((angle_points*2) /tot_threads) * num_thread;
        start_point = start_angle*mom_points;
        if (num_thread != tot_threads -1){
            end_angle = ((angle_points*2) /tot_threads) * (num_thread+1);
            end_point = end_angle*mom_points-1;
        }
        else{
            end_angle = angle_points*2;
            end_point= end_angle*mom_points-1;
        }
        slice_points = end_point-start_point +1 ;

        vec integral_Y(mom_points, fill::zeros);
        

        vec slice_k4( slice_points);        
        long long int index_GammaM, index_dx, index_sx;
        sp_mat Gamma_slice(slice_points, number_of_points*2);
        for (long long int i=0; i < slice_points; i++){
            index_GammaM = i+start_point;
            if (method == "LRBF"){
                index_sx = index_GammaM  - mom_points *( close_q/2 );
                index_dx = index_GammaM  + mom_points* ( close_q/2 ) +1;
            }
            else{
                index_sx = index_GammaM  - mom_points ;
                index_dx = index_GammaM  + mom_points +1;
            }
            if (index_GammaM > number_of_points){
                index_sx -= number_of_points;
                index_dx -= number_of_points;
            }
            if (index_dx > number_of_points || index_sx < 0 ) {
                index_sx = 0;
                index_dx = number_of_points;
            }
            for (long long int ii=index_sx; ii < index_dx ; ii++ ){
                if (GammaM(index_GammaM, ii) != 0){
                    Gamma_slice(i,ii) = GammaM(index_GammaM, ii);
                }
                if (GammaM(index_GammaM, ii+ number_of_points) != 0){
                    Gamma_slice(i,ii+ number_of_points) = GammaM(index_GammaM, ii+ number_of_points);
                }
            }
        }
        ...
        for (long long int t=1; t< total_steps+1; t++){
            ...
            k1.subvec(start_point,end_point) = Gamma_slice * prob_dens;
            #pragma omp barrier
            k2.subvec(start_point,end_point) = Gamma_slice * (prob_dens + 0.5*ts*k1 );
            #pragma omp barrier
            k3.subvec(start_point,end_point) = Gamma_slice * (prob_dens + 0.5*ts*k2 );
            #pragma omp barrier
            slice_k4 = Gamma_slice * (prob_dens + ts*k3 );
            //k4.subvec(start_point,end_point) = Gamma_slice * (prob_dens + ts*k3 );
            #pragma omp barrier

            prob_dens.subvec(start_point,end_point) = prob_dens.subvec(start_point,end_point) +  ts/6.0 * ( k1.subvec(start_point,end_point) + 2.0*k2.subvec(start_point,end_point) + 2.0*k3.subvec(start_point,end_point) +slice_k4 ) ;
            prob_dens.subvec(start_point,end_point).for_each( [](vec::elem_type& val) {
                if (val < 0){
                    val=0;
                }
            });
            if (t % normalization_step==0) {
                //normalization = norm2D_spec(angle_grid, mom_grid, prob_dens);
                for (long long int i=start_angle; i < end_angle; i++){
                    integral_Y = prob_dens.subvec(i*mom_points, (i+1)*mom_points -1);
                    mat int_trap = trapz( Y, integral_Y);
                    integral_X(i)= int_trap(0,0);
                }
                #pragma omp barrier

                if (num_thread ==0) {
                    normalization = as_scalar( trapz( X, integral_X.subvec(0, angle_points-1)) + trapz( X, integral_X.subvec(angle_points, angle_points*2-1)) ) ;
                    prob_dens = prob_dens/ normalization;
                }
...
void QSLE::print_restart(){
    ...
    /* RC 01.10.2024  - Start */
    if (!mu01_file.empty()){
        fout_R << "mu01                 " << mu01_file << endl;
    }
    if (!wrad_file.empty()){
        fout_R << "wrad                 " << wrad_file << endl;
    }
    if (!wnorad_file.empty()){
        fout_R << "wnorad               " << wnorad_file << endl;
    }
    /* RC 01.10.2024  - End */
...
sp_mat importing_Gamma_slice(long long int num_thread)
{
    string threadN;
    stringstream ss1;
    ss1 << num_thread;
    ss1 >> threadN;
    sp_mat Gamma_slice(slice_points, number_of_points*2);
    Gamma_slice.load(base_name + "_matrix_" + threadN);
